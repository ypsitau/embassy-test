...
async fn main(_spawner: Spawner) {
    ...
    let (mut cdc_acm_sender, mut cdc_acm_receiver) = {
    ...
    let fut_echo= async {
        ...
        let e = loop {
            ...
            info!("Connected");
            let e = loop {
                let buf_read = match cdc_acm_receiver.read_packet(&mut buf).await {
                    Ok(n) => &buf[..n], Err(e) => break e,
                };
                if let Err(e) = cdc_acm_sender.write_packet(buf_read).await { break e; }
            };
